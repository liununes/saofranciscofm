...
const PhoneContact = ({ position, variant = 'bar', className = '' }: PhoneContactProps) => {
  ...
  const tel = config.telefone_contato.replace(/\D/g, '');

  if (variant === 'header') {
    return (
      <a href={`tel:${tel}`} className={`flex items-center gap-1.5 text-primary-foreground/80 hover:text-primary-foreground transition-colors ${className}`}>
  ...
  if (variant === 'footer') {
    return (
      <div className={`flex items-center justify-center gap-2 mt-4 ${className}`}>
        ...
        <a href={`tel:${tel}`} className="text-primary-foreground font-semibold hover:text-secondary transition-colors">
    ...
    return (
      <div className={`bg-primary/5 border-b border-primary/10 ${className}`}>
        <div className="container mx-auto px-4 py-2 flex items-center justify-center gap-2 text-sm">
          ...
          <a href={`tel:${tel}`} className="text-primary font-semibold hover:underline">
            {config.telefone_contato}
          </a>
        </div>
      </div>
  ...
  if (variant === 'block') {
    ...
    return (
      <div className={`flex items-center ${alignClass} gap-3 py-4 px-6 bg-primary/5 rounded-xl border border-primary/10 ${className}`}>
        ...
        <a href={`tel:${tel}`} className="text-primary font-bold text-xl hover:underline">
          {config.telefone_contato}
        </a>
      </div>
    );
