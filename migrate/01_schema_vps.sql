-- ============================================================
-- SCHEMA COMPLETO PARA SUPABASE CLOUD (v3 - Ordem Corrigida)
-- Execute este SQL no SQL Editor do Studio
-- ============================================================
...
-- 2. TABELAS SEM DEPENDÊNCIAS DE FUNÇÕES

-- user_roles (precisa existir antes da função has_role)
...
-- 3. FUNÇÕES (depois que todas as tabelas existem)
...
-- 4. POLICIES (depois que tabelas E funções existem)
...
CREATE POLICY IF EXISTS "Admins can manage paginas" ON public.paginas FOR ALL USING (public.has_role(auth.uid(), 'admin'));

-- radio_config policies
...
CREATE POLICY IF EXISTS "Admins can manage programas" ON public.programas FOR ALL USING (public.has_role(auth.uid(), 'admin'));
